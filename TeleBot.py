...
diretorio = os.getenv("DIR_BASE") # Diretório usado para a função de XML
dir_temp = os.getenv("DIR_TEMP") # Diretório temporário para transformação dos arquivos xlsx
dir_excel = os.getenv("DIR_EXCEL")  # Diretório para armazenamento dos arquivos xlsx
chave_api_tel = os.getenv("TELEGRAM_TOKEN") # Token API Telebot
...
def Perguntar_Func(msg): # Perguntar Rasp -> Pergunta se o usuário deseja saber as ultimas ofertas do Mercado Livre
    ...
    markup = types.ReplyKeyboardMarkup(one_time_keyboard=True) # Criação do Teclado de Respostas Rápidas
    markup.add('Verificar Ofertas (Mercado Livre)', 'Enviar Nota Fiscal (XML)', 'Voltar')
    bot.send_message(msg.chat.id, "Selecione uma opção: ", reply_markup=markup) # Reply do BOT
...
def Alocar_Email(msg):# Alocar Email -> tem como função fiscalizar o email inserido pelo usuário e integrar o email na class thrashing de ScrapMercado
    ...
    padrao = r'^[\w\.-]+@[\w\.-]+\.\w+$' # Separação do EMAIL através do RegEx
...
def Conversor(arquivo, itens): # Conversor para criação do arquivo XML
    ...
    with open(os.path.join(diretorio,arquivo), "rb") as arquivo_xml:
